...
void writeBinaryFile(const string& inputFilename, vector<VectorElement>& symbolVector, vector<Pair>& grammar){
    ...
    switch (intSize){ 
        case 8: 
            ...
            for (int ptr = 0; ptr < symbolVector.size(); ptr++){
                if (symbolVector[ptr].symbol == EMPTY){
                    ptr = symbolVector[ptr].next;
                    if (ptr == EMPTY) break;
                } 
                outputVector_int8.emplace_back(static_cast<int8_t>(symbolVector[ptr].symbol));
            }             
        ...
        case 16:
            ...
            for (int ptr = 0; ptr < symbolVector.size(); ptr++){
                ...
                outputVector_int16.emplace_back(static_cast<int16_t>(symbolVector[ptr].symbol));
            }                  
        ...
        case 32:
            ...
            for (int ptr = 0; ptr < symbolVector.size(); ptr++){
                ...
                outputVector_int32.emplace_back(static_cast<int32_t>(symbolVector[ptr].symbol));
            }                  
...
// void handleDisappearingPair(Pair disappearingPair, VectorElement *currentPosition, vector<VectorElement>& symbolVector, HashTable& hashTable, vector<list<PairRecord>>& priorityQueueVector){

//     VectorElement* previousOccurrence = currentPosition->prev;
//     VectorElement* nextOccurrence = currentPosition->next;

//     if (previousOccurrence != nullptr){
//         previousOccurrence->next = nextOccurrence;
//         if (nextOccurrence != nullptr) nextOccurrence->prev = previousOccurrence;
//         else if (previousOccurrence->prev == nullptr) removePair(disappearingPair, hashTable, priorityQueueVector);
//         return;
//     } 
//     if (nextOccurrence == nullptr) return;
//     nextOccurrence->prev = nullptr;

//     if (nextOccurrence->next == nullptr) removePair(disappearingPair, hashTable, priorityQueueVector);
//     else {
//         auto itHashTable = hashTable.find(disappearingPair);
//         itHashTable->second.pairRecordIterator->firstAppearance = nextOccurrence;
//     }
...
// void removePair(Pair pair, HashTable& hashTable, vector<list<PairRecord>>& priorityQueueVector){
//     auto itHashTable = hashTable.find(pair);
//     auto itPairRecord = itHashTable->second.pairRecordIterator;
//     itPairRecord->pairCount = 2; //it will cause the record to be deleted in the first time it is encountered
// }
